...
def _sign_windows(artifacts: list[Path], config: dict[str, Any]) -> bool:
    ...
    for artifact in artifacts:
        if artifact.suffix.lower() in [".exe", ".dll", ".msi"]:
            cmd = ["signtool", "sign", "/f", cert_path, "/t", timestamp_url, "/v"]  # Verbose output
...
def _notarize_macos(artifact: Path, config: dict[str, Any]) -> bool:
    ...
    try:
        ...
        cmd = ["xcrun", "notarytool", "submit", str(artifact), "--keychain-profile", profile, "--wait"]
...
def _sign_linux(artifacts: list[Path], config: dict[str, Any]) -> bool:
    ...
    for artifact in artifacts:
        ...
        cmd = ["gpg", "--detach-sign", "--armor", "--local-user", gpg_key, "--output", str(sig_file), str(artifact)]
...
EXAMPLE_CONFIG = {
    "code_signing": {
        ...
        "linux": {"gpg_key_id": "your-gpg-key-id"},
    }
}
