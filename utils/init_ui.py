...
def init_ui(self):
        ...
        try:
            ...
            eff_mode = "light"
        candidates = [
            os.path.join(project_dir, "logo", "sidebar_logo2.png") if eff_mode == "dark" else os.path.join(project_dir, "logo", "sidebar_logo.png"),
        ...
        if logo_path:
            ...
            target_size = 200
            self.sidebar_logo.setPixmap(pixmap.scaled(target_size, target_size, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation))
        ...
        if self.btn_acasl_loader:
            self.btn_acasl_loader.setToolTip("Configurer les plugins ACASL (post‑compilation) à exécuter après la compilation.")
        if self.venv_button:
        ...
        try:
            import utils.engines_loader  # triggers discovery
        except Exception:
            pass
        ...
        def update_compiler_options_enabled():
            ...
            if engine_id == 'pyinstaller':
                ...
                if self.btn_select_icon:
                    self.btn_select_icon.setEnabled(platform.system() == "Windows")
                for w in [self.nuitka_onefile, self.nuitka_standalone, self.nuitka_disable_console, self.nuitka_show_progress, self.nuitka_output_dir]:
...
def show_language_dialog(self):
    ...
    if ok and choice:
        lang_pref = 'System' if choice == 'System' else next((str(x.get('code', 'en')) for x in langs if str(x.get('name', '')) == choice), 'en')
        try:
            tr = asyncio.run(_i18n.get_translations(lang_pref))
            _apply_translations(self, tr)
            try:
            ...
            if hasattr(self, 'log') and self.log:
                ...
                self.log.append(f"Langue appliquée: {getattr(meta, 'get', lambda k, d=None: d)('name', lang_pref) if isinstance(meta, dict) else lang_pref}")
...
def _is_qss_dark(css: str) -> bool:
    ...
    try:
        ...
        bg_matches = [m.group(2).strip() for m in re.finditer(r"(?i)(background(?:-color)?|window|base)\s*:\s*([^;]+);", css)]
...
def _apply_translations(self, tr: dict[str, object]) -> None:
    try:
        ...
        if getattr(self, 'select_theme', None):
            try:
                ...
                if getattr(self, 'theme', 'System') == 'System':
                    val = tr.get('choose_theme_system_button') or tr.get('choose_theme_button') or tr.get('select_theme')
                else:
        ...
        if getattr(self, 'label_workspace_section', None):
            self.label_workspace_section.setText(str(tr.get('label_workspace_section', self.label_workspace_section.text())))
        ...
        if getattr(self, 'compiler_tabs', None):
            try:
                if getattr(self, 'tab_pyinstaller', None):
                    ...
                    if idx >= 0:
                        self.compiler_tabs.setTabText(idx, str(tr.get('tab_pyinstaller', self.compiler_tabs.tabText(idx))))
        ...
        if getattr(self, 'nuitka_disable_console', None):
            self.nuitka_disable_console.setText(str(tr.get('nuitka_disable_console', self.nuitka_disable_console.text())))
        ...
        if getattr(self, 'nuitka_output_dir', None):
            try:
                self.nuitka_output_dir.setPlaceholderText(str(tr.get('nuitka_output_dir', self.nuitka_output_dir.placeholderText())))
            except Exception:
                pass
        ...
        try:
            ...
            if getattr(self, 'nuitka_disable_console', None):
                self.nuitka_disable_console.setToolTip(_tt('tt_nuitka_disable_console', self.nuitka_disable_console.toolTip()))
            if getattr(self, 'btn_nuitka_icon', None):
    ...
    try:
        ...
        try:
            ...
            if getattr(self, "sidebar_logo", None) is not None:
                ...
                project_dir = os.path.abspath(os.path.dirname(sys.argv[0]))
                candidates = [
                    os.path.join(project_dir, "logo", "sidebar_logo2.png") if effective_mode == "dark" else os.path.join(project_dir, "logo", "sidebar_logo.png"),
                    os.path.join(project_dir, "logo", "sidebar_logo.png"),
                    os.path.join(project_dir, "logo", "sidebar_logo2.png"),
                ]
                logo_path = None
