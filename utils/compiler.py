...
from PySide6.QtWidgets import (
    QMessageBox, QWidget, QVBoxLayout, QPushButton, QLabel, QCheckBox, QPlainTextEdit
)
from PySide6.QtCore import QProcess, QTimer
import json
from engine_sdk.utils import redact_secrets, clamp_text
...
def _kill_process_tree(pid: int, *, timeout: float = 5.0, log=None) -> bool:
    ...
    try:
        ...
        try:
            ...
            alive2 = []
        _log(f"✅ Process tree terminated (pid={pid})" if not alive2 else f"🛑 Process tree forced killed (pid={pid}, remaining={len(alive2)})")
        return True
    except Exception:
            ...
            try:
                ...
                def _after_acasl(_rep):
                    ...
                    try:
                        ...
                        pass
                QTimer.singleShot(0, lambda a=list(artifacts): run_post_compile_async(self, a, finished_cb=_after_acasl))
            except Exception:
...
def start_compilation_process(self, file):
    ...
    try:
        ...
        try:
            timeout_sec = int(getattr(engine, 'get_timeout_seconds', lambda _gui: int(os.environ.get('PYCOMPILER_PROCESS_TIMEOUT', '1800')))(self))
        except Exception:
        ...
        if timeout_sec and timeout_sec > 0:
            ...
            def _on_timeout(proc=process, seconds=timeout_sec):
                try:
                    self.log.append(f"⏱️ Timeout ({seconds}s) pour {getattr(proc, 'file_basename', '?')}. Arrêt en cours…")
                except Exception:
                    pass
                try:
                ...
                def _force_kill(p=proc):
                    if p.state() != QProcess.NotRunning:
                        try:
                            self.log.append(f"🛑 Arrêt forcé du processus {getattr(p, 'file_basename', '?')} après délai de grâce.")
                        except Exception:
                            pass
...
def handle_stdout(self, process):
                ...
                try:
                    ...
                    if isinstance(evt, dict):
                        ...
                        if isinstance(prog, dict):
                            cur = prog.get('current'); total = prog.get('total')
                        ...
                        if isinstance(ui_req, dict):
                            try:
                                ...
                                if isinstance(msg, dict):
                                    ...
                                    res = box.exec()
                                    result = 'yes' if (mtype in ('question','ask') and res == _QMB.Yes) else ('no' if mtype in ('question','ask') else 'ok')
                                            ...
                                            if w is not None:
                                                ...
                                                try:
                                                    if isinstance(w, QPushButton):
                                                        w.clicked.connect(lambda checked=False, wid=wid: _emit_event('clicked', wid))
                                                    elif isinstance(w, QCheckBox):
                                                        w.stateChanged.connect(lambda _s, wid=wid, w=w: _emit_event('changed', wid, {'checked': w.isChecked()}))
                                                    elif isinstance(w, QPlainTextEdit):
                                                        w.textChanged.connect(lambda wid=wid, w=w: _emit_event('changed', wid, {'text': w.toPlainText()[:5000]}))
                                                except Exception:
                                                    pass
                                        elif op == 'set':
                                            props = widget.get('props') or {}
    ...
    if "Successfully created" in data or "Nuitka: Successfully created" in data:
        ...
        if process.state() != QProcess.NotRunning:
            self.log.append("<span style='color:orange;'>ℹ️ Nuitka a signalé la fin de compilation dans le log, mais le process n'est pas terminé. Forçage du kill (arbre) et nettoyage UI...</span>")
            try:
...
def cancel_all_compilations(self):
    ...
    self.set_controls_enabled(True)
    if errors:
        self.log.append(f"❌ Certains processus n'ont pas pu être arrêtés (voir erreurs ci-dessus).")
    else:
