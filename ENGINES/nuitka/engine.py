        ...
        try:
            ...
            if not vroot:
                if vm and getattr(gui, 'workspace_dir', None):
                    ...
                    gui.log.append(_tr("❌ Aucun venv détecté. Créez un venv dans le workspace.", "❌ No venv detected. Create a venv in the workspace."))
                return False
            ...
            if vm:
                ...
                gui.log.append(_tr("🔎 Vérification de Nuitka dans le venv (asynchrone)...", "🔎 Verifying Nuitka in venv (async)..."))
                def _on_check(ok: bool):
                    try:
                        if ok:
                            gui.log.append(_tr("✅ Nuitka déjà installé", "✅ Nuitka already installed"))
                        else:
                            gui.log.append(_tr("📦 Installation de Nuitka dans le venv (asynchrone)...", "📦 Installing Nuitka in venv (async)..."))
    ...
    def program_and_args(self, gui, file: str) -> Optional[Tuple[str, List[str]]]:
        ...
        try:
            ...
            if not os.path.isfile(python_path):
                gui.log.append(_tr("❌ python non trouvé dans le venv : ", "❌ python not found in venv: ") + str(python_path))
                gui.show_error_dialog(os.path.basename(file))
                return None
