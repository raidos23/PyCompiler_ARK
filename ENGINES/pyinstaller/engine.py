...
class PyInstallerEngine(CompilerEngine):
                    ...
                    if missing:
                        ...
                        if pm:
                            if pm == "apt":
                                ...
                                packages = ["binutils", "patchelf", "p7zip-full"]
                            try:
                                gui.log.append(_tr("🔧 Dépendances système PyInstaller manquantes: ", "🔧 Missing PyInstaller system dependencies: ") + ", ".join(missing))
                            except Exception:
                                pass
                            proc = sdm.install_packages_linux(packages, pm=pm)
                            if proc:
                                try:
                                    gui.log.append(_tr("⏳ Installation des dépendances système… (attente de fin)", "⏳ Installing system dependencies… (waiting to finish)"))
                                except Exception:
                                    pass
                                try:
                                ...
                                try:
                                    gui.log.append(_tr("⛔ Installation des dépendances système annulée ou non démarrée.", "⛔ System dependencies installation cancelled or not started."))
                                except Exception:
                                    pass
                                return False
                        else:
                            try:
                                ...
                                QMessageBox.critical(
                                    ...
                                    _tr("Impossible d'installer automatiquement les dépendances système (patchelf, p7zip).", "Unable to auto-install system dependencies (patchelf, p7zip)."),
                                )
                            except Exception:
                                pass
                            return False
    ...
    def program_and_args(self, gui, file: str) -> Optional[Tuple[str, List[str]]]:
        ...
        try:
            ...
            if not vroot:
                gui.log.append(gui.tr("❌ Venv introuvable pour résoudre pyinstaller.", "❌ Venv not found to resolve pyinstaller."))
            ...
            if not os.path.isfile(pyinstaller_path):
                gui.log.append(gui.tr("❌ pyinstaller non trouvé dans le venv : ", "❌ pyinstaller not found in venv: ") + str(pyinstaller_path))
                gui.show_error_dialog(os.path.basename(file))
                return None
