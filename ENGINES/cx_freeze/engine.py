...
from engine_sdk import CompilerEngine, SysDependencyManager, resolve_project_venv, pip_executable, pip_show, pip_install
...
class CxFreezeEngine(CompilerEngine):
        ...
        try:
            ...
            try:
                if ok:
                    ...
                    gui.log.append(gui.tr(f"❌ Installation échouée ({package})", f"❌ Installation failed ({package})"))
            except Exception:
                pass
            return ok
        except Exception:
        ...
        try:
            ...
            if os_name == "Linux":
                ...
                if not python3_bin:
                    ...
                    has_python_dev = False
                    try:
                        rc = _subprocess.run([python3_bin, "-c", "import sysconfig,os,sys;p=sysconfig.get_config_h_filename();sys.exit(0 if p and os.path.exists(p) else 1)"], stdout=_subprocess.DEVNULL, stderr=_subprocess.DEVNULL)
                ...
                if has_pkgconf:
                    for pc in ("openssl", "zlib"):
                        try:
                            rc = _subprocess.run(["pkg-config", "--exists", pc], stdout=_subprocess.DEVNULL, stderr=_subprocess.DEVNULL)
                            if rc.returncode != 0:
                ...
                try:
                    rc = _subprocess.run(["bash", "-lc", "command -v ldconfig >/dev/null 2>&1 && ldconfig -p | grep -E 'libcrypt\\.so\\.1|libxcrypt' -q"], stdout=_subprocess.DEVNULL, stderr=_subprocess.DEVNULL)
                    needs_libxcrypt = (rc.returncode != 0)
                except Exception:
                    needs_libxcrypt = not (os.path.exists("/usr/lib/libcrypt.so.1") or os.path.exists("/lib/x86_64-linux-gnu/libcrypt.so.1"))
                if missing or missing_libs or needs_libxcrypt:
                    ...
                    if not pm:
                        try:
                            ...
                            QMessageBox.critical(
                                ...
                                _tr("Impossible d'installer automatiquement les dépendances système (build tools, python3-dev, pkg-config, openssl, zlib, etc.).", "Unable to auto-install system dependencies (build tools, python3-dev, pkg-config, openssl, zlib, etc.)."),
                            )
                        except Exception:
                            pass
                        return False
                    if pm == "apt":
                        packages = [
                            "build-essential", "automake", "autoconf", "libtool", "binutils", "make", "gcc", "g++",
                            "libc6-dev",
                            "python3", "python3-dev", "python3-pip",
                            "pkg-config", "libssl-dev", "zlib1g-dev", "libxcrypt1",
                            "patchelf", "p7zip-full",
                        ]
                    elif pm == "dnf":
                        packages = [
                            "gcc", "gcc-c++", "make", "binutils", "autoconf", "automake", "libtool",
                            "glibc-devel",
                            "python3", "python3-devel", "python3-pip",
                            "pkgconf-pkg-config", "openssl-devel", "zlib-devel", "libxcrypt-compat",
                            "patchelf", "p7zip",
                        ]
                    elif pm == "pacman":
                        packages = [
                            "base-devel",
                            "python", "python-pip",
                            "pkgconf", "openssl", "zlib", "libxcrypt-compat",
                            "patchelf", "p7zip",
                        ]
                    else:  # zypper
                        packages = [
                            ...
                            "glibc-devel",
                            "python3", "python3-devel", "python3-pip",
                            "pkg-config", "libopenssl-devel", "zlib-devel", "libxcrypt-compat",
                            "patchelf", "p7zip-full",
                        ]
                    try:
                        details = []
                ...
                sdm = SysDependencyManager(parent_widget=gui)
                pkgs = [
                    {"id": "Microsoft.VisualStudio.2022.BuildTools", "override": "--add Microsoft.VisualStudio.Workload.VCTools --passive --norestart"}
                ]
                p = sdm.install_packages_windows(pkgs)
                if p is not None:
                    try:
                        gui.log.append(_tr("⏳ Installation des dépendances Windows… (attente de fin)", "⏳ Installing Windows dependencies… (waiting to finish)"))
                    except Exception:
                        pass
                    try:
                        p.waitForFinished(-1)
        ...
        try:
            ...
            if not vroot:
                ...
                if vm and getattr(gui, 'workspace_dir', None):
                    ...
                    try: gui.log.append(gui.tr("❌ Aucun venv détecté. Créez un venv dans le workspace.", "❌ No venv detected. Create a venv in the workspace."))
            ...
            if vm:
                if vm.is_tool_installed(vroot, 'cx_Freeze') or vm.is_tool_installed(vroot, 'cxfreeze'):
                    return True
                try:
                    gui.log.append(gui.tr("🔎 Vérification de cx_Freeze dans le venv (asynchrone)…", "🔎 Verifying cx_Freeze in venv (async)…"))
                except Exception:
                    pass
                def _on_check(ok: bool):
                    try:
                        if ok:
                            ...
                            gui.log.append(gui.tr("📦 Installation de cx_Freeze dans le venv (asynchrone)…", "📦 Installing cx_Freeze in venv (async)…"))
    ...
    def build_command(self, gui, file: str) -> List[str]:
        ...
        try:
            if hasattr(self, '_cb_gui') and self._cb_gui and self._cb_gui.isChecked() and platform.system() == 'Windows':
                extra += ['--base-name', 'Win32GUI']
        except Exception:
            pass
        # Include dependencies toggle
        try:
            if hasattr(self, '_cb_include_deps') and self._cb_include_deps is not None and not self._cb_include_deps.isChecked():
    ...
    def program_and_args(self, gui, file: str) -> Optional[Tuple[str, List[str]]]:
        ...
        try:
            ...
            if not vroot:
                gui.log.append(gui.tr("❌ Venv introuvable pour résoudre cx_Freeze.", "❌ Venv not found to resolve cx_Freeze."))
            ...
            if not os.path.isfile(python_exe):
                gui.log.append(gui.tr("❌ python non trouvé dans le venv : ", "❌ python not found in venv: ") + str(python_exe))
                gui.show_error_dialog(os.path.basename(file))
                return None
    ...
    def create_tab(self, gui):
        ...
        try:
            from PySide6.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QLineEdit, QPushButton, QLabel, QFileDialog, QComboBox, QCheckBox
        except Exception:
            return None
        tab = QWidget()
        ...
        def _browse():
            try:
                d = QFileDialog.getExistingDirectory(tab, gui.tr("Choisir le dossier de sortie", "Choose output directory"), "")
            except Exception:
    ...
    def apply_i18n(self, gui, tr: Dict[str, str]) -> None:
        ...
        try:
            ...
            import importlib.resources as ilr, json as _json
            pkg = __package__
            lang_data = {}
            ...
            try:
                if getattr(self, '_output_dir_input', None):
                    ph = g('cx_output_dir_ph') or gui.tr("Dossier de sortie (--target-dir)", "Output directory (--target-dir)")
                    try:
            ...
            try:
                ...
                if getattr(self, '_target_name_input', None):
                    ph = g('cx_target_name_ph') or gui.tr("Nom de l'exécutable (--target-name)", "Executable name (--target-name)")
                    try:
            ...
            try:
                if getattr(self, '_cx_packages_title', None):
                    self._cx_packages_title.setText(g('packages_title') or gui.tr("Paquets à inclure", "Packages to include"))
            ...
            try:
                if getattr(self, '_cx_modules_title', None):
                    self._cx_modules_title.setText(g('modules_title') or gui.tr("Modules à inclure", "Modules to include"))
            ...
            try:
                if getattr(self, '_cx_excludes_title', None):
                    self._cx_excludes_title.setText(g('excludes_title') or gui.tr("Exclusions (modules/paquets)", "Modules/packages to exclude"))
            ...
            try:
                if getattr(self, '_cx_data_title', None):
                    self._cx_data_title.setText(g('data_title') or gui.tr("Fichiers/Dossiers à inclure (données)", "Data files/directories to include"))
            ...
            try:
                if getattr(self, '_cx_zip_include_title', None):
                    self._cx_zip_include_title.setText(g('zip_include_title') or gui.tr("Patterns d'inclusion ZIP", "Zip include patterns"))
            ...
            try:
                if getattr(self, '_cx_zip_exclude_title', None):
                    self._cx_zip_exclude_title.setText(g('zip_exclude_title') or gui.tr("Patterns d'exclusion ZIP", "Zip exclude patterns"))
            ...
            try:
                if getattr(self, '_cx_replace_title', None):
                    self._cx_replace_title.setText(g('replace_paths_title') or gui.tr("Remplacer chemins", "Replace paths"))
                if getattr(self, '_replace_paths_edit', None):
                    ph = g('cx_replace_paths_ph') or gui.tr("pattern=>replacement, séparés par des virgules", "pattern=>replacement, comma-separated")
                    try:
            ...
            try:
                ...
                if getattr(self, '_constants_edit', None):
                    ph = g('cx_constants_ph') or gui.tr("NAME=VALUE, séparés par des virgules", "NAME=VALUE, comma-separated")
                    try:
